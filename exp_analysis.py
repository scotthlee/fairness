...
exp = pd.read_csv('data/exp_stats.csv')
...
factors = [
    'loss', 'goal', 'class_balance', 
    'group_balance', 'pred_bias'
]
...
loss = "C(loss, Treatment(reference='micro')) +"
goal = " C(goal, Treatment(reference='odds')) +"
group_balance = " C(group_balance, Treatment(reference='no_minority')) +"
class_balance = " C(class_balance, Treatment(reference='balanced')) +"
pred_bias2 = " C(pred_bias, Treatment(reference='low'))"
pred_bias3 = " C(pred_bias, Treatment(reference='low_one'))" 
f2 = loss + goal + group_balance + class_balance + pred_bias2
f3 = loss + goal + group_balance + class_balance + pred_bias3

micro_mod2 = smf.ols('rel_acc_diff ~ ' + f2, data=exp2)
micro_res2 = micro_mod2.fit()
micro_mod3 = smf.ols('rel_acc_diff ~ ' + f3, data=exp3)
...
mm_tpr_mod2 = smf.ols('rel_mean_tpr_diff ~ ' + f2, data=exp2)
mm_tpr_res2 = mm_tpr_mod2.fit()
mm_tpr_mod3 = smf.ols('rel_mean_tpr_diff ~ ' + f3, data=exp3)
...
mx_tpr_mod2 = smf.ols('max_tpr_diff ~ ' + f2, data=exp2)
mx_tpr_res2 = mx_tpr_mod2.fit()
mx_tpr_mod3 = smf.ols('max_tpr_diff ~ ' + f3, data=exp3)
